...
export default function TriageAssistantPage() {
	...
	const handleGradeSubmit = async (grading: ClinicalGrading) => {
		if (!user || !user?.loggedIn || !user.data || !currentScenario) {
			return false;
		}
	...
	async function handleAddScenario(data: TriageData): Promise<boolean> {
								...
								<CardContent className="space-y-4">
									<div className="grid grid-cols-2 gap-4">
										...
										<div>
											...
											<p className="font-semibold">
												{currentScenario.content?.gender
													?.split("")[0]
													.toUpperCase() || "-"}
											</p>
										</div>
						...
						<div className="hidden lg:block">
							<div className="sticky top-6">
								<GradingSection
									...
									aiResposnse={currentScenario.aiResponse}
								/>
							</div>
						</div>
					)}
