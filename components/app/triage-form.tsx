...
function TriageForm({
	...
	const TriageFormSchema = z.object({
		...
		chiefComplaint: z.object({
			title: z
				...
				.min(2, "Chief complaint is too short"),
			description: z
				.string("Chief complaint description is required")
				.min(1, "Chief complaint description is too short")
				.regex(
					/^\s*(?:\S+\s+){4,}\S+/s,
					"Chief complaint description must be at least 5 words",
				),
		}),
		modeOfArrival: z
	...
	const { control, setValue, reset, handleSubmit, trigger } =
		useForm<FormSchema>({
			...
			<form
				onSubmit={async (e) => {
					...
					try {
						...
						toast.loading("Submitting triage entry...");
						...
						toast.dismiss();
						toast.success("Triage entry submitted successfully!");
						setPending(false);
					} catch (error) {
						toast.dismiss();
						setPending(false);
						...
						<Controller
							...
							render={({ field, fieldState }) => (
								<Field>
									...
									<FieldDescription>
										Format: Drug - Occurence (e.g.
										Acetaminophen - twice daily)
									</FieldDescription>
									<AddChipInput
